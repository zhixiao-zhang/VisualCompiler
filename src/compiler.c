	...
	while (temp != NULL) {
		if (!strcmp(temp->name, val->content)) {
			if (inStruc && LCnum) // 当前变量是结构体域
			{
				if (!temp->inStruc) {
					// 结构体域与变量重名
					printf("Error type 9 at Line %d:Struct "
					       "Field and Variable use "
					       "the same _tokenName.\n",
					       yylineno);
				} else if (temp->inStruc &&
					   temp->structNum != strucNum) {
					// 不同结构体中的域重名
					printf("Error type 10 at Line %d:Struct "
					       "Fields use the same "
					       "_tokenName.\n",
					       yylineno);
				} else {
...
void newfunc(int num, ...)
{
	...
	case 1:
		...
		for (i = 0; i < rnum; i++) {
			if (rtype[i] == NULL ||
			    strcmp(rtype[i], functail->rtype))
				printf("Error type 12 at Line %d:Func "
				       "return type error.\n",
				       yylineno);
		}
...
void getdetype(pnode val)
{
	...
	if (val != NULL) {
		if (!strcmp(val->_tokenName, "ParamDec")) {
			functail->va_type[functail->va_num] =
				val->cld[0]->content;
			functail->va_num++;
			return;
		}
...
InterCode translate_Stmt(pnode Stmt)
{
	...
	// Compst
		 ...
		 !strcmp((Stmt->cld)[0]->_tokenName, "Compst")) {
		// Preorder((Stmt->cld)[0],0);
		return translate_CompSt((Stmt->cld)[0]);
	}
	// RETURN Exp SEMI
	else if (Stmt->ncld == 3 &&
		 !strcmp((Stmt->cld)[0]->_tokenName, "RETURN")) {
		// 中间代码优化
	...
	// IF LP Exp RP Stmt
		...
		InterCode code1 =
			translate_Cond((Stmt->cld)[2], lable1, lable2);
	...
	// IF LP Exp RP Stmt ELSE Stmt
		...
		return add_Codes(7, code1, new_lable_Code(lable1), code2,
				 new_goto_Code(lable3), new_lable_Code(lable2),
				 code3, new_lable_Code(lable3));
	}
	// WHILE LP Exp RP Stmt
	else if (Stmt->ncld == 5 &&
		 !strcmp((Stmt->cld)[0]->_tokenName, "WHILE")) {
		Operand lable1 = new_lable();
		Operand lable2 = new_lable();
		Operand lable3 = new_lable();
		InterCode code1 =
			translate_Cond((Stmt->cld)[2], lable2, lable3);
		InterCode code2 = translate_Stmt((Stmt->cld)[4]);
		return add_Codes(6, new_lable_Code(lable1), code1,
				 new_lable_Code(lable2), code2,
				 new_goto_Code(lable1), new_lable_Code(lable3));
	}
	return new_Code();
}
...
Operand get_Operand(pnode Exp)
{
	...
	// MINUS Exp  (Exp:INT)
		 ...
		 !strcmp((Exp->cld)[0]->_tokenName, "MINUS")) {
...
InterCode translate_Exp(pnode Exp, Operand place)
{
	...
	// Exp1 ASSIGNOP Exp2
		 ...
		 !strcmp((Exp->cld)[1]->_tokenName, "ASSIGNOP")) {
		// Exp1 -> ID
		if ((Exp->cld)[0]->ncld == 1 &&
		    !strcmp(((Exp->cld)[0]->cld)[0]->_tokenName, "ID")) {
			Operand variable =
				new_Variable(((Exp->cld)[0]->cld)[0]->content);
			Operand existOp = get_Operand((Exp->cld)[2]);
			// 中间代码优化
			if (existOp == NULL) {
				Operand t1 = new_tempvar();
				InterCode code1 =
					translate_Exp((Exp->cld)[2], t1);
				InterCode code2 = new_assign_Code(variable, t1);
				if (place == NULL)
					return add_Codes(2, code1, code2);
				else {
					InterCode code3 = new_assign_Code(
						place, variable);
					return add_Codes(3, code1, code2,
							 code3);
				}
			} else {
	...
	// Exp MINUS Exp
		 ...
		 !strcmp((Exp->cld)[1]->_tokenName, "MINUS")) {
		Operand op1 = get_Operand((Exp->cld)[0]);
		Operand op2 = get_Operand((Exp->cld)[2]);
		if (op1 != NULL && op2 != NULL) {
			InterCode code3 = new_Code();
	...
	// Exp RELOP Exp
		 ...
		 !strcmp((Exp->cld)[1]->_tokenName, "RELOP")) {
		isCond = 1;
	}
	// NOT Exp
	...
	if (isCond) {
		...
		InterCode code2 =
			add_Codes(2, new_lable_Code(lable1),
				  new_assign_Code(place, new_Const(1)));
		return add_Codes(4, code0, code1, code2,
				 new_lable_Code(lable2));
	}
	return new_Code();
}
...
InterCode translate_Cond(pnode Exp, Operand lable_true, Operand lable_false)
{
	...
	if (Exp->ncld == 3 && !strcmp((Exp->cld)[1]->_tokenName, "RELOP")) {
		...
		if (op1 == NULL && op2 == NULL) {
			...
			return add_Codes(3, code1, code3,
					 new_goto_Code(lable_false));
		} else if (op1 != NULL && op2 == NULL) {
			Operand t2 = new_tempvar();
			InterCode code2 = translate_Exp((Exp->cld)[2], t2);
			...
			code3->operands.jump.op2 = t2;
			return add_Codes(3, code2, code3,
					 new_goto_Code(lable_false));
		} else if (op1 != NULL && op2 != NULL) {
			code3->operands.jump.op1 = op1;
			code3->operands.jump.op2 = op2;
	...
	// Exp AND Exp
		...
		Operand lable1 = new_lable();
		InterCode code1 =
			translate_Cond((Exp->cld)[0], lable1, lable_false);
		InterCode code2 =
	...
	// Exp OR Exp
		...
		Operand lable1 = new_lable();
		InterCode code1 =
			translate_Cond((Exp->cld)[0], lable_true, lable1);
		InterCode code2 =
			translate_Cond((Exp->cld)[2], lable_true, lable_false);
		return add_Codes(3, code1, new_lable_Code(lable1), code2);
	}
...
char *Int2String(int num, char *str) // 10进制
{
	...
	for (; j < i / 2; j++) {
		//对称交换两端的值
		//其实就是省下中间变量交换a+b的值：a=a+b;b=a-b;a=a-b;
	...
	case _ADD: {
		...
		if (op2->kind == CONSTANT) {
			fprintf(fp, "    addi %s, %s, %d\n",
				allocate_reg(result), allocate_reg(op1),
				op2->value);
		} else {
